...
## for Row 49, the last name has been dropped. R has considered only the first dash
```
...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = "" # Puts "HEY!" between the pieces (default is underscore) & "" makes it look normal with single space
        ) 
...
```
```{r}
my_dates1 <- c(03221988, 09302020, 12041995)
my_dates_converted1 <- mdy(my_dates1)
my_dates_converted1
class(my_dates_converted1)
```
...
```{r}
# Your code here: 
some_dates <- c?la(11061972, 02142007,01301843)
